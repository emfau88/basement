...
try {
  ...
  await desktop.locator('canvas').click({ position: { x: 520, y: 460 } })
  ...
  assert(await desktop.locator('#projectTitle').textContent() === 'MarschLegenden', 'Desktop MarschLegenden card did not open')
  ...
  assert(await desktop.locator('#projectTitle').textContent() === 'Voidline: Farhaven', 'Desktop Archive hover/click did not target the fourth grave')
  await desktop.locator('#projectClose').click()
  await desktop.locator('#inspectBack').click()
  ...
  assert(await mobile.locator('.mobile-project-card').count() === 9, 'Games carousel does not contain nine projects')
  ...
  assert(await mobile.locator('#mobileSheetToggle').getAttribute('aria-expanded') === 'false', 'Games inspect did not clear the Mobile sheet')
  await mobile.keyboard.press('Escape')
  ...
  await mobile.locator('#mobileSheetToggle').click()
  assert(await mobile.locator('#mobileGameInspect').textContent() === 'View project cemetery', 'Archive inspect action is missing')
  await mobile.locator('#mobileGameInspect').click()
  assert(await mobile.locator('body').evaluate((body) => body.classList.contains('inspect-selector')), 'Mobile Archive cemetery inspect view did not open')
  assert(await mobile.locator('#inspectBack').textContent() === '← ARCHIVE LOUNGE', 'Mobile Archive inspect view has the wrong return label')
  assert(await mobile.locator('#mobileSheetToggle').getAttribute('aria-expanded') === 'false', 'Archive inspect did not clear the Mobile sheet')
  await mobile.locator('#inspectBack').click()
  assert(!(await mobile.locator('body').evaluate((body) => body.classList.contains('inspect-selector'))), 'Mobile Archive inspect did not return to the lounge')
  await mobile.locator('#mobileSheetToggle').click()
