...
const outputDirectory = path.resolve('docs/qa/bulk-23-6-archive-contact-ao')
await mkdir(outputDirectory, { recursive: true })

const server = await createServer({ server: { host: '127.0.0.1', port: 0 } })
await server.listen()
const address = server.httpServer?.address()
if (!address || typeof address === 'string') throw new Error('Unable to resolve the contact AO QA server port')
...
try {
  ...
  for (const variant of [
    ...
    { name: 'after', query: '?contact-ao=pilot', expected: 'pilot' },
  ]) {
    ...
    const runtimeState = await page.locator('canvas').getAttribute('data-archive-contact-ao')
    ...
    const estimatedGpuBytes = Number(await page.locator('canvas').getAttribute('data-archive-contact-ao-bytes') ?? 0)
...
console.log(`Captured Archive contact AO A/B comparison in ${outputDirectory}`)
